...
class ConformalController:
    ...
    def __call__(self, pos_x, pos_y, orientation_z, linear_x, angular_z, boxes, tracking_res, predictions, goal):
        paths, vels = self.generate_paths(pos_x, pos_y, orientation_z, n_skip=5)
        # paths, vels = self.generate_paths_wheel_vel(pos_x, pos_y, orientation_z, linear_x, angular_z)
        safe_paths, vels = self.filter_unsafe_paths(paths, vels, boxes, tracking_res, predictions)
    ...
    def filter_unsafe_paths(self, paths, vels, boxes, tracking_res, predictions):
        """
        ...
        coeff = 1 - np.exp(-self._lambda)
        reach = .15 * np.arange(self._n_steps)
        mask_p_per_path = np.zeros((n_paths,), dtype=bool)
        for obj_id, prediction in predictions.items():
            object_pos = tracking_res[obj_id][-1]

            paths_shifted_and_scaled = (paths[:, 1:, :] - prediction) / coeff + prediction
            obj_mask = np.any(np.sum((paths_shifted_and_scaled - object_pos) ** 2, axis=-1) < (ROBOT_RAD + .1 / np.sqrt(2.) + reach) ** 2, axis=-1)
