...
def linear_extrapolation(traj, prediction_len, dt=0.1, noise_floor=0.01):
    """
    Simple linear extrapolation function.
    Generates a future trajectory of length prediction_len using the mean
    position of the last 3 frames and the mean velocity of the recent 3 frames.
    """
    if traj.shape[0] >= 3:
        ...
        if np.linalg.norm(v_mean) < 1e-3:
            v_mean = v_mean + noise_floor  # Minimum noise floor
...
class TrajectronPredictor(BasePredictors):
    ...
    def __call__(self, tracking_results):
        ...
        # Perform prediction through the Trajectron model
